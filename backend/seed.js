...
const weighted = (pairs) => {
  ...
  for (const [label, weight] of pairs) {  // ✅ label first, weight second
    if ((r -= weight) <= 0) return label;  // ✅ returns the label
  }
  return pairs[0][0];  // ✅ returns the first element (label)
};
...
const NOTE_TEMPLATES = [
  ...
  (c) =>
    `Comparing us against a competitor on price. Emphasise support SLA and onboarding.`,
  (c) =>
...
const run = async () => {
  ...
  let user = await User.findOne({ email: "alex@timetoprogram.com" });
  if (user) {
    await Promise.all([
